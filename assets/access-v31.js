...
function openSession(account){
 ...
 location.href='centro-interno.html';
}
...
function render(){
 ...
 const active=validSession();if(active){location.replace('centro-interno.html');return;}
 const account=read(ACCOUNT_KEY);
 const first=!account;
 root.innerHTML=`<div class="v31-access-card"><div class="v31-access-brand"><img src="assets/logo-lockup.svg" alt="El Errante"><span>Sistema interno · V3.1</span></div><div class="v31-access-copy"><p class="eyebrow">Acceso usuarios</p><h1>${first?'Configura el primer acceso local.':'Bienvenido de nuevo.'}</h1><p>${first?'Este navegador todavía no tiene un usuario interno. Crea las credenciales que usarás para entrar a Operación y Finanzas.':'Ingresa con tu usuario para abrir el centro interno y elegir el módulo de trabajo.'}</p></div><form id="v31-access-form" class="v31-access-form">${field('Usuario','username','text',`autocomplete="username" required value="${esc(account?.username||'')}"`)}${field('Contraseña','password','password','autocomplete="current-password" minlength="8" required')}${first?field('Confirmar contraseña','confirm','password','autocomplete="new-password" minlength="8" required'):''}<button class="v31-primary" type="submit">${first?'Crear acceso y entrar':'Ingresar al sistema'}</button><div id="v31-access-message" class="v31-form-message" aria-live="polite"></div></form><div class="v31-access-security"><strong>Acceso local protegido</strong><p>La contraseña no se guarda: el navegador conserva únicamente un derivado PBKDF2 con sal aleatoria. Esta capa protege la experiencia local, pero GitHub Pages sigue siendo un host estático; la autorización servidor/RLS se activará al migrar a Supabase.</p></div></div>`;
...
window.EL_ERRANTE_ACCESS_V31={version:VERSION,accountKey:ACCOUNT_KEY,sessionKey:SESSION_KEY,validSession};
