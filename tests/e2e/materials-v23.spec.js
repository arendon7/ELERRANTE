...
test.describe('Materias primas e inventario inteligente V2.3',()=>{
  test('convierte pedidos en productos, materiales y costo provisional',async({page})=>{
    ...
    await expect(page.locator('html')).toHaveAttribute('data-materials-version','2.3.0');
  });
  ...
  test('distingue inventario desconocido de cero físico confirmado',async({page})=>{
    await seed(page);
    await page.goto('/admin.html');
    await page.getByRole('button',{name:'Abrir simulación local'}).click();
    const panel=page.locator('#materials-v23');
