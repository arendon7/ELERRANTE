(()=>{
  ...
  const INTERNAL_PAGES=new Set(['centro-interno','admin','activacion','control','operacion','studio','actas','presentacion']);
  const INTERNAL_TARGETS=new Set(['centro-interno.html','admin.html','activacion.html','control.html','operacion.html','studio.html','actas.html','presentacion.html']);
  ...
  function observe(){
    const observer=new MutationObserver(records=>{for(const record of records){for(const node of record.addedNodes){if(node.nodeType!==Node.ELEMENT_NODE)continue;recover(node.matches?.('img[src],source[srcset]')?node.parentElement||document:node);}}});
  ...
  function add(container,selector,label,href,beforeSelector,className=''){
    ...
    const link=document.createElement('a');link.href=href;link.textContent=label;if(className)link.className=className;
    const before=beforeSelector?container.querySelector(beforeSelector):null;container.insertBefore(link,before||null);
  }
  ...
  function curatePublicNav(container,mobile=false){
    ...
    add(container,'a[href="nosotros.html"]','Nuestra cocina','nosotros.html','a[href="bitacora.html"]',cls);
  ...
  function removeInternalPublicLinks(){
    document.querySelectorAll('#site-footer a, main a[data-internal-only]').forEach(link=>{if(INTERNAL_TARGETS.has(hrefOf(link)))link.remove();});
  }
  ...
  function enhance(){
    ...
    if(hosted&&!isInternal){
      ...
      document.querySelectorAll('.demo-badge').forEach(element=>{const text=(element.textContent||'').toLowerCase();if(text.includes('gold master')||text.includes('demo')||text.includes('sin internet')||text.includes('biblioteca editorial completa'))element.remove();});
    }

    if(isInternal){
      document.querySelectorAll('a[href="equipo.html"]').forEach(link=>{link.href='centro-interno.html';if((link.textContent||'').trim().toLowerCase()==='equipo')link.textContent='Centro interno';});
    }else{
