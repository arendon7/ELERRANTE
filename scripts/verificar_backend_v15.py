...
for path in ["assets/commerce-v15.css","tests/e2e/commerce-v14.spec.js","backend/supabase/schema-v14.sql","documentacion/ROADMAP_OPERACION_COMERCIAL_V14.md"]:
    text(path)

require(config, 'version: "2.5.0"', "La configuración comercial no declara V2.5.0.")
...
require(config, 'dataStatus: "ESTIMADO"', "La configuración no identifica el carácter provisional de los gastos.")
...
if 'assets/commerce-v14.js' in admin_html: PROBLEMS.append("Admin todavía carga el runtime administrativo legado V1.4.")
...
require(worker, 'el-errante-v2-5-visual-1', "Service worker no usa caché V2.5.0.")
...
require(host, 'PUBLIC_VERSION="2.5.0"', "Host mode no declara V2.5.0.")
require(host, 'ACTIVE_CACHE="el-errante-v2-5-visual-1"', "Host mode no apunta a la caché V2.5.0.")
...
require(health, 'el-errante-v2-5-visual-1', "Public Health no verifica la caché V2.5.0.")

for label, content in {"runtime":runtime,"config":config,"checkout":checkout,"admin":admin,"admin.html":admin_html,"checkout.html":checkout_html}.items():
    lowered = content.lower()
    if "service_role" in lowered: PROBLEMS.append(f"Se encontró service_role en superficie pública: {label}.")
    if "supabase_service" in lowered: PROBLEMS.append(f"Se encontró un identificador de secreto de servidor en {label}.")
...
print("Gastos activos: etapa piloto provisional")
