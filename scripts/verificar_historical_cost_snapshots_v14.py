...
bridge_gate=read('scripts/verificar_master_cost_bridge_v13.py')
workflow=read('.github/workflows/historical-cost-v14.yml')
marker=read('historical-cost-version.txt')

require("const VERSION='1.4.0'" in core,'Core V1.4 no declara versión 1.4.0')
for token in ("EVENT_KEY='ee_v14_cost_snapshot_events'","STATE_KEY='ee_v14_cost_snapshot_state'","MATERIALIZATION_KEY='ee_v12_cost_materialization_events'"):
    require(token in core,f'Falta clave V1.4: {token}')
for token in ('materialAt','signatureAt','productCostAt','productSnapshot','captureOrder','capturePurchase','captureMovement','historicalOrder','historicalPurchase','historicalMargin','scanNewFacts'):
    require(token in core,f'Falta contrato V1.4: {token}')
for token in ('CANONICAL_BASELINE','MATERIALIZED_STANDARD','LEGACY_EMBEDDED','OBSERVED_AT_MOVEMENT','UNKNOWN','OBSERVED_ONLY_STANDARD_UNKNOWN'):
    require(token in core,f'Falta semántica de origen histórico: {token}')
require("row?.type==='MATERIALIZED'" in core,'V1.4 no reconstruye materializaciones del ledger V1.2')
require('event._time<=cutoff' in core,'V1.4 no filtra revisiones por fecha efectiva')
...
require('localStorage.setItem(KEYS.orders' not in core and 'localStorage.setItem(KEYS.purchases' not in core,'V1.4 no puede reescribir hechos fuente')
...
): require(token in test,f'Falta regresión V1.4: {token}')
...
for token in ('Certificar costo histórico V1.4','historical-cost-version.txt','EXPECTED_SHA','historical-cost-snapshots-v14.js','finance-historical-cost-v14.js','operacion.html','finanzas.html'):
    require(token in workflow,f'Workflow V1.4 incompleto: {token}')
require('historical_cost_snapshots=v1.4.0' in marker,'Marcador público no declara V1.4')
require('legacy_backfill=forbidden' in marker and 'unknown_cost=explicit' in marker,'Marcador público no declara reglas de migración')
...
print('certification=composable_plus_public_health')