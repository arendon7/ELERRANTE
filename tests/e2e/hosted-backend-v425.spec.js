...
test.describe('V4.2.5 · backend público hospedado con kill switch comercial',()=>{
  ...
  test('Pages conecta backend pero mantiene comercio desactivado y no inventa banco',async({page})=>{
    const {config}=await loadSynthetic(page,'https://arendon7.github.io/ELERRANTE/config-truth.html');
    expect(config.ordering.commerceEnabled).toBe(false);
    expect(config.payment.bank).toBe('');
    expect(config.payment.accountType).toBe('');
    expect(config.payment.accountNumber).toBe('');
    expect(config.payment.key).toBe('');
  });
  ...
  test('demo operativo interno conserva backend comercial apagado incluso en Pages',async({page})=>{
