...
test.describe('V4.2.5 · barrera de activación comercial',()=>{
  test('backend conectado sin datos de pago mantiene el checkout protegido',async({page})=>{
    ...
    await page.evaluate(()=>{
      window.EL_ERRANTE_COMMERCE_CONFIG={
        ...
        ordering:{commerceEnabled:false}
      };
    ...
    const state=await page.evaluate(()=>({
      ...
      enabled:window.EE_PUBLIC_COMMERCE_GUARD_V29.commerceEnabled(),
  ...
  test('medio de pago real no abre checkout mientras el GO comercial siga apagado',async({page})=>{
    await page.goto('/checkout.html');
    await page.waitForFunction(()=>Boolean(window.EE_PUBLIC_COMMERCE_GUARD_V29));
    ...
    expect(state.enabled).toBe(false);
    expect(state.checkout).toBe(false);
    expect(state.marker).toBe('pilot-locked');
    await expect(page.locator('#ee-address')).toHaveCount(0);
    await expect(page.locator('#ee-receipt')).toHaveCount(0);
  });

  test('checkout queda listo sólo con backend, pago y GO comercial explícito',async({page})=>{
    await page.goto('/checkout.html');
    await page.waitForFunction(()=>Boolean(window.EE_PUBLIC_COMMERCE_GUARD_V29));
